...
async function loadTeachers() {
  ...
  // Auto-fill read-only school info fields
  if (!isAdmin) {
    const school = currentSchool || allSchoolsCache.find(s => s.emis_number === emis);
    if (school) {
      const zoneEl = document.getElementById('t-zone-display');
      const emisEl = document.getElementById('t-emis-display');
      const schoolEl = document.getElementById('t-school-display');
      if (zoneEl) zoneEl.value = school.zone || '';
      if (emisEl) emisEl.value = school.emis_number || '';
      if (schoolEl) schoolEl.value = school.school_name || '';
    }
  }
...
function editTeacher(id) {
  ...
  // Fill auto display fields too
  const zoneEl = document.getElementById('t-zone-display');
  const emisEl = document.getElementById('t-emis-display');
  const schoolEl = document.getElementById('t-school-display');
  if (zoneEl) zoneEl.value = t.zone || '';
  if (emisEl) emisEl.value = t.emis_number || '';
  if (schoolEl) schoolEl.value = t.school_name || '';
...
function resetTeacherForm() {
  ...
  // Re-fill school display fields after clear
  const emis = currentProfile?.emis_number;
  const school = currentSchool || allSchoolsCache.find(s => s.emis_number === emis);
  if (school) {
    const zEl = document.getElementById('t-zone-display');
    const eEl = document.getElementById('t-emis-display');
    const sEl = document.getElementById('t-school-display');
    if (zEl) zEl.value = school.zone || '';
    if (eEl) eEl.value = school.emis_number || '';
    if (sEl) sEl.value = school.school_name || '';
  }
...
async function exportTeachers() {
  ...
  const rows = [['Zone','EMIS Number','School Name','Dev','Surname','Name','Sex',
    'Standard Teaching','Grade','National ID','Emp No','Intake','DOB',
    'Qualification','Date First Appointment','Date Present Grade',
    'Village','T/A','District','Marital Status','Phone','Remarks','Responsibility']];

  (data || []).forEach(t => {
    rows.push([
      t.zone, t.emis_number, t.school_name,
      t.dev_drop, t.surname, t.name, t.sex,
      t.standard_teaching, t.grade,
      t.national_id, t.emp_number, t.intake,
      ...
      t.marital_status, t.phone, t.remarks, t.responsibility
    ]);
  });
...
// ============================================================
// TEACHERS - CSV IMPORT (Admin only)
// ============================================================

function downloadTeacherTemplate() {
  const headers = 'emis_number,zone,school_name,surname,name,sex,responsibility,standard_teaching,dev_drop,grade,qualification,emp_number,national_id,intake,dob,date_first_appointment,date_present_grade,village,traditional_authority,district,marital_status,phone,remarks';
  const example1 = '500164,CHIGONTHI,Chigonthi Primary School,BANDA,GRACE,F,Headteacher,STD 8,1-8,TL,Degree,EMP001,12345678A9,2010,1980-01-15,2005-03-01,2015-07-01,Chigonthi,Kasungu,Lilongwe,Married,0881234567,Available';
  const example2 = '500400,CHOWO,Chowo Primary School,PHIRI,JAMES,M,Teacher,STD 4,1-4,TK,Diploma,EMP002,98765432B1,2015,1985-06-20,2010-01-10,2018-04-15,Chowo,Lilongwe,Lilongwe,Single,0991234567,Available';
  const csv = headers + '
' + example1 + '
' + example2;
  ...
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'llre_teachers_template.csv';
  a.click();
...
async function handleTeacherCSVUpload(event) {
  ...
  const statusEl  = document.getElementById('teacher-csv-status');
  const progressEl = document.getElementById('teacher-csv-progress');
  const bar        = document.getElementById('teacher-csv-bar');
  const text       = document.getElementById('teacher-csv-text');
  const pct        = document.getElementById('teacher-csv-pct');
  ...
  for (let fi = 0; fi < files.length; fi++) {
    ...
    bar.style.width = Math.round((fi / files.length) * 100) + '%';
    pct.textContent  = Math.round((fi / files.length) * 100) + '%';
    ...
    try {
      ...
      const BATCH = 50;
      for (let i = 0; i < rows.length; i += BATCH) {
        const batch = rows.slice(i, i + BATCH).map(r => mapTeacherCSVRow(r)).filter(Boolean);
        if (!batch.length) { totalSkipped += BATCH; continue; }

        const { error } = await db.from('teachers').upsert(batch, {
          onConflict: 'emp_number',
          ignoreDuplicates: false
        });

        if (error) {
          // emp_number may be blank — try plain insert
          const { error: err2 } = await db.from('teachers').insert(batch);
          if (err2) { totalErrors += batch.length; }
  ...
  const alertType = totalErrors > 0 && totalInserted === 0 ? 'alert-error' : 'alert-success';
  const icon      = totalErrors > 0 && totalInserted === 0 ? '⚠' : '✓';
  statusEl.innerHTML = `<div class="alert ${alertType}" style="margin-top:10px;"><span>${icon}</span>
    Import complete: <strong>${totalInserted.toLocaleString()} teachers saved</strong>
    ${totalSkipped > 0 ? `, ${totalSkipped} skipped` : ''}
    ${totalErrors  > 0 ? `, ${totalErrors} errors`   : ''}.
  </div>`;

  toast(`Imported ${totalInserted} teachers`, 'success');
  event.target.value = '';
  loadTeachers();
}

function mapTeacherCSVRow(r) {
  const surname = (r.surname || r.Surname || r.SURNAME || '').toString().trim().toUpperCase();
  const name    = (r.name    || r.Name    || r.NAME    || r['first name'] || '').toString().trim().toUpperCase();
  const emis    = (r.emis_number || r['emis number'] || r.emis || '').toString().trim();

  if (!surname || !name || !emis) return null;

  // Auto-fill zone and school_name from schools cache if missing
  const cached = allSchoolsCache.find(s => s.emis_number === emis);
  const zone       = (r.zone        || r.Zone        || cached?.zone        || '').toString().trim().toUpperCase();
  const schoolName = (r.school_name || r.school      || r['school name']    || cached?.school_name || '').toString().trim();
  ...
  return {
    ...
    surname,
    name,
    sex:                    (r.sex || r.Sex || '').toString().trim().toUpperCase() || null,
    responsibility:         r.responsibility  || r.Responsibility  || null,
    standard_teaching:      r.standard_teaching || r['standard teaching'] || null,
    dev_drop:               r.dev_drop  || r['dev drop']  || null,
    grade:                  r.grade     || r.Grade        || null,
    qualification:          r.qualification || r.Qualification || null,
    emp_number:             (r.emp_number || r['emp no'] || r['emp number'] || '').toString().trim() || null,
    national_id:            (r.national_id || r['national id'] || '').toString().trim() || null,
    intake:                 (r.intake || '').toString().trim() || null,
    dob:                    parseDate(r.dob || r.DOB),
    date_first_appointment: parseDate(r.date_first_appointment || r['date of first appointment']),
    date_present_grade:     parseDate(r.date_present_grade     || r['date of present grade']),
    village:                (r.village || r.Village || '').toString().trim() || null,
    traditional_authority:  (r.traditional_authority || r['traditional authority'] || r.ta || r.TA || '').toString().trim() || null,
    district:               (r.district || r.District || '').toString().trim() || null,
    marital_status:         r.marital_status || r['marital status'] || null,
    phone:                  (r.phone || r.Phone || r['phone number'] || '').toString().trim() || null,
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // Attach enter key to email and password fields
